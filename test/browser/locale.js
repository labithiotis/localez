...
(function () {
    ...
    Locales.prototype.load = function (lang, src) {
        var uri = src ? src : this.langDir + lang + this.langExt;
    ...
    function Locale(locale) {
        ...
        this.options = extend({
            ...
            numbers: function numbers(number, gender) {
                if (!number || number <= 0) return this.options.numbersEnums.zero;
                var b = number % 10;
                return ~ ~(number % 100 / 10) === 1 ? this.options.numbersEnums.many : b === 1 ? this.options.numbersEnums.one : b === 2 ? this.options.numbersEnums.two : b === 3 ? this.options.numbersEnums.few : this.options.numbersEnums.other;
            },
            numberToString: function numberToString(n, gender) {
                if (n === 0) return 'zero';
    ...
    Locale.prototype.parse = function (string, data) {
        ...
        if (data && typeof data !== 'object') return console.error('Second argument should to be a {object}');
    ...
    Locale.prototype.getLocaleString = function (key) {
        this.log('Get translation string from locale resource');
        return this.locale ? this.locale[String(key).hashCode()] || key : key;
    };

    // Starts the parsing of the string
    Locale.prototype.parseString = function (string, result, data, gender) {
        ...
        if (expressions && expressions.length > 0) {
            ...
            var nextExpression = expressions[0],
                indexOfOpenExpression,
                indexOfCloseExpression,
                index;

            indexOfOpenExpression = string.indexOf(this.options.openTag);

            // nested expression check
            // close off each open expression until count equals 0
            try {
            ...
            result += this.parseExpression(nextExpression, data, gender);
            ...
            return this.parseString(string, result, data, gender);
        } else {
    ...
    Locale.prototype.parseExpression = function (expression, data) {
        ...
        try {
            ...
            this.log('Determine type for this expression is: %s', type);
    ...
    Locale.prototype.extractComponentsInExpression = function (expression) {
        ...
        // remove variable length from expression string to find sections
    ...
    Locale.prototype.parseExpressionGender = function (expression, data) {
        ...
        var components = this.extractComponentsInExpression(expression),
            variable = data ? data.getDescendant(components.variable) : null,
            option = components.options[variable];

        this.log(components);

        // returns the options for male or female if found
        return option ? this.parseString(option, null, data, variable) : '';
    };

    // Parse Expression type integer
    Locale.prototype.parseExpressionInteger = function (expression, data, gender) {
        ...
        numberKey = this.options.numbers.apply(this, [number, gender]);
        result = components.options[numberKey] || components.options.other;

        this.log(components);

        if (result.match(/%i/gi)) {
            return result ? this.parseString(result.replace(/%i/gi, number.toString()), null, data) : '';
        } else {
            return result ? number + ' ' + this.parseString(result, null, data) : '';
        }
    };

    // Parse Expression type number
    Locale.prototype.parseExpressionNumber = function (expression, data, gender) {
        ...
        if (result.match(/%i/gi)) {
            return result ? this.parseString(result.replace(/%i/gi, this.options.numberToString(number, gender)), null, data) : '';
        } else {
            return result ? this.options.numberToString(number, gender) + ' ' + this.parseString(result, null, data) : '';
        }
    };
